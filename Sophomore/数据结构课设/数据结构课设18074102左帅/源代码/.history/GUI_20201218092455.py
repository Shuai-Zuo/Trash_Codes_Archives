...
class Review(QtWidgets.QMainWindow, Ui_review):
    ...
    def ConfirmClicked(self):
        self.review=self.Enter.toPlainText()
        self.my_Signal.emit(self.review)
        self.close()
    #确定按钮被点击
...
class MyWindow(QMainWindow, Ui_FSS):
    ...
    def Show_film(self):
        send = self.sender()
        self.currentfilm=send.node.film
        if send.node is not None:
            send.node.film.clicked()
            self.s.refresh_list_external(self.s.clicks_sort,send.node.film)
            _translate = QtCore.QCoreApplication.translate
            self.current_url=send.node.film.url
            if send.node.film.review[0]=='noreview':
                ...
                self.imformation.setHtml('电影名称:'+send.node.film.name+'<br />'+\
                    ...
                    '评分:'+send.node.film.rating+'<br />'+\
                    '剧情梗概:'+send.node.film.plot+'<br />'+\
                    '影评:<br />'+'<br />'.join(send.node.film.review)+'<br />'+\
                    '<p style=\" margin-top:0px; margin-bottom:0px; margin-left:0px; margin-right:0px; -qt-block-indent:0; text-indent:0px;\"><a href=\"'+\
                    send.node.film.url+\
                    '\"><span style=\" text-decoration: underline; color:#0000ff;\">'+\
                    send.node.film.url+'</span></a></p>')
    ...
    def Show_review(self):
        self.rev=Review()
        self.rev.my_Signal.connect(self.AddReview)
        self.rev.show()
    #显示影评输入界面
    def AddReview(self):
        if self.currentfilm != None:
            if self.currentfilm.review[0]=='noreview':
                self.currentfilm.review=[]
                self.currentfilm.review.append(self.rev.Enter.toPlainText())
            else:
                self.currentfilm.review.append(self.rev.Enter.toPlainText())
    #为当前电影添加影评
