...
class AVLTree:
    ...
    def get_film_from_node(self,film_name):
        ...
        if s is not None:
            ...
            return film('noname','norating','noregion','nointro','notypes','nosuchdirector','noactor','nolanguage','notime','nourl','nope')
    ...
    def _preorder_traversal_save(self,node,film):
        if node is not None:
            ...
            film.write('#')
            film.write(str(node.film.click_times))
            film.write('#')
            film.write('/'.join(node.film.review))
            film.write('\n')
    ...
    def preorder_traversal_search(self,node,search_str):
        if node is not None:
            ...
            if i == 1:
                ...
                node.film.clicked()
                self.need_refresh_list.append(node)
