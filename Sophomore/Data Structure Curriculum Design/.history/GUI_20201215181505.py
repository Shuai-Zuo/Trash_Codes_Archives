...
#        self.SearchButton.clicked.connect(self.SearchButtonClicked)
    ...
    def Show_film(self):
        ...
        if send.node is not None:
            self.imformation.self.imformation.setHtml(_translate("FSS",'电影名称:'+send.node.film.name+ \
                '\n导演:'+send.node.film.director+ \
                '\n演员:'+' '.join(send.node.film.actors)+ \
                '\n类型:'+' '.join(send.node.film.types)+ \
                '\n上映日期: '+send.node.film.time+ \
                '\n语言:'+' '.join(send.node.film.language)+ \
                '\n区域:'+' '.join(send.node.film.region)+ \
                '\n评分:'+send.node.film.rating+ \
                '\n剧情梗概:'+send.node.film.plot+'\n'+\
                '<p style=\" margin-top:0px; margin-bottom:0px; margin-left:0px; margin-right:0px; -qt-block-indent:0; text-indent:0px;\"><a href=\"'+\
                send.node.film.url+\
                '\"><span style=\" text-decoration: underline; color:#0000ff;\">aa</span></a></p>'))
        else:
    ...
    def openlinks(self):
        webbrowser.open('https://movie.douban.com/subject/3793023/')

if __name__ == '__main__':
    app = QApplication(sys.argv)
    myWin = MyWindow()
    myWin.show()
    h=myWin.s.rating_sort.gnext(myWin.s.rating_sort.head.filmlist.head)
    h1=myWin.s.rating_sort.gnext(h)
    h2=myWin.s.rating_sort.gnext(h1)
    h3=myWin.s.rating_sort.gnext(h2)
    sys.exit(app.exec_())