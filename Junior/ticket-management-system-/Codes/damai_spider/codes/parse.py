...
class Spider(object):
    ...
    def parse(self,pages:int=1):
        page_wr=open("C:\\Users\\admin\\git\\ticket-management-system-\\Codes\\damai_spider\\codes\\1.txt", "r",encoding='utf-8')
        dict_data = json.loads(page_wr.read())
        #dict_data = json.loads(self.get(pages).text)
        #print(dict_data["pageData"]["resultData"])
        need_spider_data = dict_data["performBases"]
        for datas in need_spider_data:
            datas = dict(list(datas.items())+list(datas["performs"][0].items()))
            datas.pop("performs")
...
if __name__ == '__main__':
    ...
    #spider.run()
    original_data=[]
    original_data.extend(spider.parse(1))
